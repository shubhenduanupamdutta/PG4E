"""A basic admin tool for Deno KV using Python and web services."""
# ruff: noqa: C901, PLR0912, PLR0915, PLR2004, E722, S110, S310, BLE001
...
APP_URL = os.environ["DENO_WEBAPP_URL"]
TOKEN = os.environ["TOKEN"]


def urlerror(e: Exception, url: str | None = None, *, quit_on_error: bool = False) -> None:
...
def pretty_json(status: int, text: str) -> None:
...
def add_token(url: str, secrets: dict) -> str:
    """Add authentication token to URL."""
    return url + "?token=" + secrets["token"]


def read_json(prompt: str) -> str:
...
while True:
    ...
    if len(pieces) >= 2 and pieces[0] == "set":
        url = APP_URL + "/kv/set" + pieces[1] + "?token=" + TOKEN
        if len(pieces) == 3:
            text = pieces[2]
        else:
            text = read_json("Enter json (finish with a blank line:")
    ...
    if len(pieces) == 2 and (pieces[0] == "get" or pieces[0] == "list"):
        url = APP_URL + "/kv/" + pieces[0] + pieces[1] + "?token=" + TOKEN
        if show_url:
            print(url)
